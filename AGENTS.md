...
├── AGENTS.md         # this file — canonical agent guide (only instruction file in git)
├── CLAUDE.md         # ← generated by do-stow.sh, gitignored (symlink → AGENTS.md)
└── …                 # other agent symlinks, also generated and gitignored
```
...
cursor     ~/.cursor/rules      .cursor/rules/dotfiles.md
codex      ~/.codex/skills      -
```

   - `skills_path` — where skill symlinks are deployed
   - `instruction_symlink` — path **relative to dotfiles root** for a symlink → `AGENTS.md`; use `-` if the agent reads `AGENTS.md` directly (e.g. Codex)

2. Add the instruction symlink path to `.gitignore` (it's a generated file, not committed):
...
3. Run `./do-stow.sh` — it creates both the skill symlinks and the instruction symlink automatically.

4. Commit only `skills/.agents` and `.gitignore`.
